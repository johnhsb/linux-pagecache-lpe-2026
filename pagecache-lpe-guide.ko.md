...
**문서 목적**: 페이지 캐시를 제자리(in-place) 쓰기 대상으로 오용하는 이 8개 취약점의 기술적 원리와 공통점을 이해하고, 정식 패치 적용 방법과 유의사항을 정리하며, 정식 패치가 즉시 불가능한 운영 장비를 위해 [mitigate-cve-2026.sh](mitigate-cve-2026.sh) 완화 스크립트를 안전하게 활용하는 절차를 제공한다. **스크립트는 이 중 6개(31431/43284/43500/46300/43503/46331)를 자동화 지원한다** — 나머지 2개(PinTheft/FUSE)는 5절에 수동 완화 절차만 정리했다(5.6절).
...
- **정식 패치가 최선이며, 완화 스크립트는 그 전까지의 압박 완화책이다.** [mitigate-cve-2026.sh](mitigate-cve-2026.sh)의 모듈 차단 방식은 Red Hat이 RHSB-2026-003/RHSB-2026-008에서 직접 제시한 절차와 동일하다. 단, DirtyClone의 근본 결함은 커널 코어에 있어 모듈 차단만으로는 확인된 트리거(esp4/esp6)만 막을 뿐 완전한 해결책이 아니며, **PinTheft/FUSE 2건은 스크립트가 아직 자동화하지 않는다**(5절, 5.6절).
...
- **"RHEL 계열이니 미해당"이라는 일반화는 배포판마다 검증 없이는 성립하지 않는다.** PinTheft(43494)는 정식 RHEL·CloudLinux는 미해당이지만, **Oracle Linux(UEK)는 미해당이 아니다** — 2026-07-23 참조 서버(Oracle Linux 9.8 UEK) 직접 확인 결과 `CONFIG_RDS=m`/`CONFIG_RDS_RDMA=m`/`CONFIG_RDS_TCP=m`이었다. RDS는 원래 Oracle RAC 데이터베이스 인터커넥트용 프로토콜이라, Oracle Linux가 이를 빼놨을 리 없다는 점을 검증 없이 놓쳤던 것이다(2.7절, 4.4절).
...
| CVE-2026-46331 | pedit COW | tc(traffic control) `act_pedit` | `act_pedit` | 7.8(kernel.org) / 6.7(Red Hat) | 2026-06-16 | 2026-06-16 | ✅ (v2.8.0부터) | v7.1-rc7 (도입 899ee91156e5, 수정 2bec122b9fb9 외) |
...
- **스크립트 지원 여부**: 이 저장소의 [mitigate-cve-2026.sh](mitigate-cve-2026.sh)는 `act_pedit`를 자동 지원한다 — 자동로드 차단·언로드는 esp4/esp6/rxrpc/algif_aead와 동일한 방식이며, tc pedit 사용 신호(`tc actions show action pedit`)도 함께 탐지한다(5.1절). 실서버(Oracle Linux 9.8 UEK)에서 진단(`--dry-run --verbose`) 경로를 검증했다 — 해당 서버는 이미 46331이 패치돼 있어 실제 차단/언로드/원복까지는 이 CVE 자체로는 재현하지 못했고, 그 부분은 esp4/esp6로 검증한 동일 로직(모듈 배열 기반 제네릭 처리)에 의존한다.
...
- **트리거 조건**: `CONFIG_RDS`/`CONFIG_RDS_TCP`가 활성화된 커널이어야 하고, 공개 PoC는 io_uring `IORING_REGISTER_CLONE_BUFFERS`가 존재하는 커널 6.13 이상을 요구한다.
- **RDS는 원래 Oracle이 RAC(Real Application Clusters) 데이터베이스 노드 간 인터커넥트용으로 만든 프로토콜이다.** `rds_rdma`가 InfiniBand/RDMA 기반 클러스터 통신을 담당한다. 이 특성 때문에 "RDS 모듈이 컴파일돼 있다"는 것이 곧 "실제 운영 DB 클러스터일 가능성"을 시사할 수 있다 — 아래 RHEL/Oracle Linux 항목 참고.
- **RHEL(정식 제품) 영향**: **RHEL 6~10 전 버전 공식 "Not affected"**(Red Hat 보안 데이터 API — `package_state` 전부 Not affected, `affected_release` 없음). Red Hat 자체 커널 소스 config에 `# CONFIG_RDS is not set`이라 모듈 자체가 존재하지 않고, `socket()` 호출이 즉시 `EAFNOSUPPORT`를 반환한다.
- **⚠️ Oracle Linux(UEK)는 위 "RHEL 미해당"에 포함되지 않는다 — 직접 검증 결과 CONFIG_RDS=m이다.** 2026-07-23 참조 서버(Oracle Linux 9.8, 커널 `5.15.0-322.203.3.4.el9uek.x86_64`)에서 `grep CONFIG_RDS /boot/config-$(uname -r)`로 직접 확인한 결과 `CONFIG_RDS=m`, `CONFIG_RDS_RDMA=m`, `CONFIG_RDS_TCP=m` 세 모듈 전부 컴파일돼 있었다. Oracle이 자사 RAC 제품을 위해 만든 프로토콜이니 Oracle Linux가 이를 빼놨을 리 없다는 점에서 당연한 결과지만, 이전 버전 문서에서는 이를 검증 없이 "RHEL 계열"로 뭉뚱그려 잘못 기술했었다. **"RHEL 계열이니 미해당"이라고 일반화하지 말고, 배포판별로 개별 확인할 것.**
- **완화책**: RDS가 실제로 컴파일된 배포판(Ubuntu, Oracle Linux 등)에서는 `rds`/`rds_tcp`/`rds_rdma` 모듈 차단이 커뮤니티에서 검증된 완화책으로 문서화돼 있음(5.6절). Red Hat 자체 완화책 필드는 "이용 가능한 완화책 없음/기준 미충족"이라 되어 있는데, 이는 RHEL(정식 제품) 자체가 애초에 미해당이라 나온 보일러플레이트로 판단된다 — RHEL 미해당 사실과 혼동하지 말 것.
- **"기타" 등급인 이유는 낮은 위험도가 아니라 신뢰할 수 있는 사용 신호 탐지 방법이 아직 없기 때문이다.** 자세한 내용과 스크립트 미자동화 사유는 5.6절 참고.
...
**[mitigate-cve-2026.sh](mitigate-cve-2026.sh)가 위 2~3번을 자동화한다 — 단 6개 CVE(원 5개 + pedit COW)에 한해서다.** 레드햇 계열은 `rpm -q --changelog`, 데비안/우분투 계열은 dpkg changelog를 조회한다(5.1절). PinTheft/FUSE는 스크립트가 아직 확인하지 않으므로 수동으로 위 절차를 적용해야 한다(5.6절).
...
> ⚠️ **위 표는 RHEL(Red Hat Enterprise Linux) 정식 제품에만 해당한다 — CentOS Stream처럼 사실상 동일하게 패치되는 배포판은 별개로, Oracle Linux 같은 "RHEL 계열" 파생 배포판까지 표에 적힌 "미해당" 판정이 그대로 이어진다고 가정하지 말 것.** 실제로 PinTheft(43494)는 **Oracle Linux(UEK)에서 미해당이 아니다** — 참조 서버(Oracle Linux 9.8, 커널 `5.15.0-322.203.3.4.el9uek.x86_64`)를 2026-07-23 직접 확인한 결과 `CONFIG_RDS=m`/`CONFIG_RDS_RDMA=m`/`CONFIG_RDS_TCP=m`으로 세 모듈 모두 컴파일돼 있었다(2.7절). "미해당" 판정은 반드시 실제 대상 배포판 기준으로 개별 확인할 것.
...
> **범위 안내**: 이 5절(5.1~5.5)은 스크립트가 실제로 자동화하는 **6개 CVE**(31431/43284/43500/46300/43503/46331)에 한정된다. 추가로 확인된 PinTheft(43494)·FUSE 캐시 오버플로(31694) 2건은 스크립트가 아직 다루지 않으며, 수동 완화 절차는 5.6절에 별도로 정리했다.
...
| `algif_aead` 자동로드 차단 + 언로드 | 31431 | 동일 방식. **KEV 등재 최우선 건이라 조치 대상이면 나머지 모듈과 분리해 독립적으로 먼저 확인을 묻는다**(5.3절) |
| `act_pedit` 자동로드 차단 + 언로드 | 46331 | 동일 방식. tc pedit 사용 신호(`tc actions show action pedit`)도 함께 탐지 |
| 빌트인(=y) 감지 및 경고 | 6개 전체(트리거 경로 판단용) | `/boot/config-$(uname -r)` 확인 |
| **배포판 패치 자동 확인 + 이미 패치된 모듈 스킵** | 6개 전체 | 레드햇 계열: `rpm -q --changelog` / 데비안·우분투 계열: dpkg changelog. 전부 오프라인(기본값). 4.3/4.4절 참고 |
| **벤더 보안 어드바이저리 조회(`--online`)** | 6개 전체 | 레드햇 계열: `dnf updateinfo`로 "수정본 배포됐으나 미설치"인 CVE를 취약 확정. changelog에 CVE ID가 없는 커널(Oracle UEK 등)에서 유일한 확정 수단. 데비안 계열: `apt changelog` 폴백. 4.2절 참고 |
| **VPN/IPsec·RxRPC/AFS·tc pedit·컨테이너 사용 신호 탐지(참고 정보)** | 의사결정 보조 | `ip xfrm state/policy`, IKE 포트, strongswan/libreswan 서비스, AFS 마운트, `tc actions show action pedit`, docker/podman/containerd/kubelet 등 확인(전부 휴리스틱, 차단 여부를 대신 결정하지 않음) |
| 비특권 user/net 네임스페이스 제한(선택) | 43284 / 46300 / 43503 / 46331 (ESP·act_pedit 경로). **31431에는 효과 없음** | `kernel.unprivileged_userns_clone=0` / `user.max_user_namespaces=0`. 적용 전 원래 값을 설정 파일 주석에 기록 |
...
3. **변경 이력 기록을 위해 로그를 남기며 실제 적용**:
   ...
   ```
4. `algif_aead`(Copy Fail)가 조치 대상이면 **먼저 독립적으로** 차단 여부를 묻는다(KEV 등재 최우선 건이라는 시급성과, 실사용이 드물어 위험도가 낮다는 점을 함께 안내). 이어서 나머지 조치 대상 모듈을 그룹으로 한 번 더 묻는다 — 한쪽만 승인해도 승인된 것만 적용되므로, 다른 모듈이 망설여진다는 이유로 Copy Fail 조치까지 함께 무산되지 않는다. 승인된 모듈만으로 차단 설정 파일 생성(`/etc/modprobe.d/mitigate-cve-2026.conf`) → 로드된 모듈 언로드 → 사후 검증까지 자동 수행된다. 설정 파일에는 생성 시각·호스트명·커널 버전·스크립트 버전이 주석으로 기록되어 변경 추적이 가능하다.
...
### 5.6 스크립트 미지원 2건 — 수동 완화 절차

PinTheft(43494)·FUSE 캐시 오버플로(31694)는 [mitigate-cve-2026.sh](mitigate-cve-2026.sh)가 아직 자동화하지 않는다. 아래는 벤더·커뮤니티가 문서화한 수동 절차이며, **적용 전 반드시 사전 조건(5.2절과 동일한 원칙 — 실사용 여부 확인)을 직접 확인할 것.** 자동 사전 점검·사용신호 탐지·롤백 안전장치가 없으므로 5.1~5.5절보다 신중하게 접근해야 한다.

**제외 사유는 CVE마다 다르다.**

- **PinTheft(43494)**: **자동 차단의 위험이 비대칭적으로 크기 때문**에 제외했다. RDS는 원래 Oracle RAC 데이터베이스 클러스터 인터커넥트용으로 설계된 프로토콜이라, `rds`/`rds_rdma`/`rds_tcp`가 컴파일된 서버는 실제 운영 중인 DB 클러스터일 가능성을 배제할 수 없다(2.7절). esp4/esp6(`ip xfrm`)·rxrpc(AFS 마운트)·act_pedit(`tc actions show`)처럼 신뢰할 수 있는 사용 신호 탐지 방법(활성 RDS 소켓 확인, Oracle Clusterware 프로세스·패키지 흔적 확인 등)이 아직 설계·검증되지 않았다. 이런 상태로 자동 차단을 추가하면, 이미 자동화된 6건보다 오히려 더 위험한 침묵의 실패(운영 DB 클러스터 인터커넥트 단절)를 유발할 수 있어 신중하게 보류했다.
- **FUSE 캐시 오버플로(31694)**: 반대로 **실사용 범위가 이 스크립트의 "차단 기본값" 전제에 맞지 않아서** 제외했다. `fuse` 모듈은 sshfs·rclone·다수의 Snap 패키지·데스크톱 클라우드 동기화 도구가 의존하는 범용 모듈이라, esp4/esp6/rxrpc/act_pedit/algif_aead처럼 "대부분 서버에서 미사용"이라는 전제가 성립하지 않는다(2.8절). 또한 이 CVE 자체가 커널 6.16-rc1 이상에서만 발현해, RHEL을 포함한 주요 안정화 배포판이 현재 이 코드 경로를 아예 포함하지 않으므로 자동화 긴급성도 낮다. 권장 완화책(`fusermount3` setuid 제거, 마운트 정책 제한)도 "모듈 하나 차단"이라는 이 스크립트의 패턴과 결이 달라 수동 절차로 남겼다.

#### PinTheft (CVE-2026-43494)

**RHEL(정식 제품)·CloudLinux는 이미 미해당**(CONFIG_RDS 자체가 없음, 2.7절) — 단 **Oracle Linux(UEK)는 미해당이 아니다**(CONFIG_RDS=m 직접 확인, 2.7절). 이 절차는 RDS가 실제로 컴파일된 배포판(Ubuntu, Oracle Linux 등)에 해당하며, **적용 전 RDS 실사용 여부(활성 소켓, Oracle Clusterware/DB 클러스터링 소프트웨어 흔적 등)를 반드시 직접 확인할 것** — 이 스크립트에 자동 통합하지 않은 이유이기도 하다(5.6절 서두 참고).
...
- [ ] 적용 시점 기준으로 [CISA KEV 카탈로그](https://www.cisa.gov/known-exploited-vulnerabilities-catalog)에서 8개 CVE의 등재 여부를 다시 조회했는가 (1.1절 표는 조사 시점 스냅샷)
- [ ] 대상 서버가 스크립트 지원 6개 CVE(31431/43284/43500/46300/43503/46331) 각각에 대해 실제로 패치되어 있는지 배포판 어드바이저리로 개별 확인했는가 (버전 비교 아님, 4.2~4.4절 참고)
...
- [ ] **추가 2건(PinTheft/FUSE)도 패치 여부를 확인했는가** — 스크립트가 자동 확인하지 않으므로 배포판 어드바이저리로 수동 확인 필요 (1절, 2.7~2.8절)
...
- [CISA KEV 카탈로그](https://www.cisa.gov/known-exploited-vulnerabilities-catalog) — 적용 시점에 8개 CVE를 재조회할 것
